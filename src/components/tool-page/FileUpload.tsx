...
const FileUpload = ({ onFileChange }: FileUploadProps) => {
  ...
  return (
    <div className="drop-zone border-2 border-dashed rounded-lg p-8 text-center mb-6">
      ...
      <p className="text-gray-600 mb-2 dark:text-gray-400">{t("common.drag_and_drop")}</p>
      <p className="text-gray-500 text-sm mb-4">{t("common.or")}</p>
      <label htmlFor="file-upload" className="bg-blue-600 text-white px-6 py-2 rounded-lg cursor-pointer hover:bg-blue-700 transition duration-300">
        {t("common.browse_files")}
      </label>
      <input id="file-upload" type="file" className="hidden" multiple onChange={onFileChange} accept=".pdf" />
