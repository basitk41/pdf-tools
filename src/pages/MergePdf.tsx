  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        {files.length > 0 && (
          <div className="flex flex-col gap-2 mb-4">
            {files.map((file, index) => (
              <div key={index} className="flex items-center justify-between p-2 border rounded-md dark:bg-gray-700">
        ...
        {!mergedPdf && (
          <Button onClick={handleMerge} disabled={files.length < 2 || isProcessing} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition duration-300">
            {isProcessing ? `${t("common.processing")} (${progress}%)` : t("tools.merge_pdf.title")}
          </Button>
        )}
        ...
        {mergedPdf && (
          <div className="mt-6 p-4 bg-green-50 rounded-lg text-center dark:bg-green-900/20">
            <X className="text-green-500 w-10 h-10 mx-auto mb-3" />
            <h3 className="text-lg font-semibold text-gray-800 mb-2 dark:text-white">{t("common.conversion_complete")}</h3>
            <p className="text-gray-600 mb-4 dark:text-gray-400">{t("common.conversion_success_message")}</p>
